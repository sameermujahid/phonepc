...
if ($result->num_rows > 0) {
    ...
    // Update the create_status when the form is submitted
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        $sql = "UPDATE add_task SET create_status = 1 WHERE Task_id = '$selected_task_id'";

        if ($conn->query($sql) === TRUE) {
            echo "Task accepted successfully.";
        } else {
            echo "Error accepting task: " . $conn->error;
        }
    }

    // HTML form to display the details and accept the task
    echo '
    <form action="acceptemptask.php?Task_id=' . $selected_task_id . '&id=' . $selected_id . '" method="post">
        <div class="row">
            ...
            <div>
                Current File: <a type="button" class="btn btn-success" href="downloadfile.php?file_id=' . $selected_task_id . '">Download File</a>
            </div>
        </div>
            <div class="mb-3 col-md-12">
            ...
            <div class="mb-3 col-md-12">
                ...
                <input type="submit" value="Accept Task" class="btn btn-primary">
            </div>
        </div>
    </form>';
} else {
