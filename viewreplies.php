					...
					<div class="col-xl-12 col-lg-12">
						<div class="card">
							...
							<div class="card-body">
                                ...
                                <?php
// Assuming you have retrieved the selected Task_id from the previous page or URL parameter
...
$stmt = $conn->prepare("SELECT id, Task_name, Person_id, Person_name FROM add_task WHERE Task_id = ?");
...
if ($result->num_rows > 0) {
    ...
    $taskId = 'SA_TASK_' . str_pad($row['id'], 2, '0', STR_PAD_LEFT);
    ...
    // Retrieve the replies for the selected task
    $replyStmt = $conn->prepare("SELECT Date_of_reply, replies FROM reply_task WHERE Task_id = ?");
    $replyStmt->bind_param("s", $selected_task_id);
    $replyStmt->execute();
    $replyResult = $replyStmt->get_result();

    if ($replyResult->num_rows > 0) {
        echo '<div class="row"><div class="mb-3 col-md-12"><h3>Replies:</h3></div></div>';

        while ($replyRow = $replyResult->fetch_assoc()) {
            $dateOfReply = $replyRow['Date_of_reply'];
            $replyText = $replyRow['replies'];

            echo '
            <div class="row">
                <div class="mb-3 col-md-12">
                    <label class="form-label">Date of Reply</label>
                    <input type="text" class="form-control" value="' . $dateOfReply . '" disabled>
                </div>
                <div class="mb-3 col-md-12">
                    <label class="form-label">Reply</label>
                    <textarea class="form-control" disabled>' . $replyText . '</textarea>
                </div>
            </div>';
        }
    } else {
        echo '<div class="row"><div class="mb-3 col-md-12"><p>No replies found for the selected task.</p></div></div>';
    }

    // Close the prepared statement for replies
    $replyStmt->close();
} else {
    echo '<div class="row"><div class="mb-3 col-md-12"><p>No task found with the selected ID.</p></div></div>';
}

// Close the prepared statement for task details
$stmt->close();
